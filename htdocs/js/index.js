...
//				window.test;
		...
		function show(platform,part){
                        platform=unescape(platform);
			if (platform=="all"){
			platform=$("input[name='platform']").serialize();
			}
                        part=unescape(part);
                                 $.ajax({
						type:'post',
							...
							var arry=[];
								...
								'<tr>'+
							        '<td class="table1" id="'+id+'-tool" >'+id+'</td>'+
                                                                '<td class="table1"><a href="javascript:void(0);" onclick="show('+"'"+"all"+"',"+"'"+escape(data[index][id].part)+"'"+' )" id="'+id+'-part" style="text-decoration:none;">'+data[index][id].part+'</a></td>'+
								'</tr>'+
       								'<tr>'+
								...
								'<li id="'+id+'-lot" >'+data[index][id].lot+'</li>'+
                                                                '<li id="'+id+'-remain" >'+'Remain:'+data[index][id].remain+'</li>'+
				...
				$("img").on("mouseenter",function(){
				 ...
				 window.timeer=setInterval("refresh('"+escape(platform)+"','"+escape(part)+"')",60000);
                                                        },
